...
class ServerSettingsFragment : Fragment() {
    ...
    private fun showProfileDialog(existing: ServerProfile?) {
        ...
        val timeout = view.findViewById<TextInputEditText>(R.id.timeoutInput)
        val headers = view.findViewById<TextInputEditText>(R.id.headersInput)
        val basicAuth = view.findViewById<MaterialSwitch>(R.id.basicAuthSwitch)
        val basicAuthUser = view.findViewById<TextInputEditText>(R.id.basicAuthUserInput)
        val basicAuthPass = view.findViewById<TextInputEditText>(R.id.basicAuthPassInput)
        val basicAuthUserLayout =
            view.findViewById<com.google.android.material.textfield.TextInputLayout>(R.id.basicAuthUserLayout)
        val basicAuthPassLayout =
            view.findViewById<com.google.android.material.textfield.TextInputLayout>(R.id.basicAuthPassLayout)

        basicAuth.setOnCheckedChangeListener { _, checked ->
            basicAuthUserLayout.visibility = if (checked) View.VISIBLE else View.GONE
            basicAuthPassLayout.visibility = if (checked) View.VISIBLE else View.GONE
        }
        ...
        existing?.let {
            ...
            if (it.requestTimeout > 0) timeout.setText(it.requestTimeout.toString())
            headers.setText(it.customHeaders)
            basicAuth.isChecked = it.basicAuth
            basicAuthUser.setText(it.basicAuthUsername)
            basicAuthPass.setText(it.basicAuthPassword)
        }
        ...
        dialog.getButton(androidx.appcompat.app.AlertDialog.BUTTON_POSITIVE)
            .setOnClickListener {
                ...
                val profile = ServerProfile(
                    ...
                    basicAuthUsername = basicAuthUser.text?.toString()?.trim().orEmpty(),
                    basicAuthPassword = basicAuthPass.text?.toString().orEmpty(),
                )
